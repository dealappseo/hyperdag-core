...
const issues = [
{
  title: "Improve epistemic_uncertainty: add logprob entropy support",
  label: "good first issue",
  body: `Currently epistemic_uncertainty uses hedge keyword density as a proxy
...
please share it here. All perspectives welcome.`
},
{
  title: "Validate phyllotaxis rule placement vs uniform grid",
  label: "help wanted",
  body: `CommaANFIS uses phyllotaxis (golden angle 137.508°) for fuzzy
...
not the same as correctness. All perspectives welcome.`
},
{
  title: "Verify N ∈ {5,7,12} constraint for generalized comma formula",
  label: "research, mathematics",
  body: `The generalized Pythagorean Comma formula:
  C(N) = (3/2)^N / 2^k
...
{
  title: "Wire 5-signal extractor into CommaANFIS forward pass",
  label: "good first issue",
  body: `Two components exist but are not yet connected:
...
domain signals from claim text, please suggest it. All perspectives welcome.`
},
{
  title: "Ethics layer design: measurement vs. definition",
  label: "help wanted",
  body: `The RepID ethics layer maps three principles to measurable signals:
...
We are looking for dialogue. All perspectives welcome.`
}
];

for (const issue of issues) {
  try {
     const bodyArg = issue.body.replace(/"/g, '\\"');
     console.log('Creating: ', issue.title);
     execSync(`gh issue create -R DealAppSeo/hyperdag-core --title "${issue.title}" --body "${bodyArg}" --label "${issue.label || ''}"`);
  } catch(e) { console.error('Failed to create:', issue.title, e.message); }
}
